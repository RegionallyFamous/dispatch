# Dispatch for Telex Documentation

Use this wiki for all user-facing product documentation.

## Start here

- [Getting Started](https://github.com/RegionallyFamous/dispatch/wiki/Getting-Started)  
  Install Dispatch, connect Telex, and verify first success.

## Daily use

- [Managing Projects](https://github.com/RegionallyFamous/dispatch/wiki/Managing-Projects)  
  Install, update, remove, pin, snapshot, and organize projects.

## Operations and recovery

- [Troubleshooting](https://github.com/RegionallyFamous/dispatch/wiki/Troubleshooting)  
  Fast triage for install/update/connection issues.
- [Site Health and Diagnostics](https://github.com/RegionallyFamous/dispatch/wiki/Site-Health-and-Diagnostics)  
  Understand health checks and operational state.
- [Multisite Setup](https://github.com/RegionallyFamous/dispatch/wiki/Multisite-Setup)  
  Network-level behavior and responsibilities.

## Reference

- [WP-CLI Reference](https://github.com/RegionallyFamous/dispatch/wiki/WP-CLI-Reference)
- [Security Model](https://github.com/RegionallyFamous/dispatch/wiki/Security-Model)
- [Architecture](https://github.com/RegionallyFamous/dispatch/wiki/Architecture)

## Contributing and policies

- [Contributing](https://github.com/RegionallyFamous/dispatch/wiki/Contributing)
- [Security Policy](https://github.com/RegionallyFamous/dispatch/blob/main/SECURITY.md)
- [Changelog](https://github.com/RegionallyFamous/dispatch/blob/main/CHANGELOG.md)
